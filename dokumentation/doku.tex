...
Das Endprodukt dieser Arbeit ist eine full-stack Webapplikation, die auf einem Linode Server läuft. 
...
\subsubsection{Komponentenübersicht}
...
Eine Detailansicht (AufgDetails) für jede Aufgabe erfolgt in einer Modal Ansicht (Fenster innerhalb des Browsers erscheint), wobei neben dem Aufgabentext auch die Lösung, die Kategoriezugehörigkeit, der Schwierigkeitsgrad und der Autor der Aufgabe zu sehen ist. Die Aufgabe kann auch in dieser Ansicht zum Warenkorb hinzugefügt werden oder aus dem Warenkorb gelöscht werden.
...
\item Aufgaben editieren (EditForm)
...
\item Aufgaben finden (FindExSubCat, FindExBySearchText):
Dieser Bereich umfasst zwei Komponenten, die unterschiedliche Methoden zur Aufgabensuche anbieten. Mit FindExSubCatak können Aufgaben nach Kategorien und Unterkategorien gesucht werden. (Siehe Kapitel Zustandsmanagement). 
Über eine API Anfrage \texttt{(askServer(`/api/ex?cat={selectedCategory}\&subcat={subcategoryName}`, "GET"))}
Mit FindExBySearchText kann eine Aufgabe nach einer textbasierten Suchanfrage (inputValue) gesucht werde mit einer API Anfrage \texttt{(askServer(`/api/ex?search=\${inputValue}`, "GET")}


 
\item Benutzerregistrierung und -anmeldung (User Registration/Login):
Eine Form-Komponente unterstützt die Registrierung neuer Benutzer und die Anmeldung bestehender Benutzer durch Übermittlung ihrer Anmeldeinformationen an das Backend \texttt{(askServer(/api/user, "POST", user))}. Authentifizierungsdaten werden überprüft, um Zugang zu geschützten Bereichen der Anwendung zu gewähren.
...
\subsubsection{Zustandsmanagement: useState, useEffect und Signals inkl. Code-Beispielen}
