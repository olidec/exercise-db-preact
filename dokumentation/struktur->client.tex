...
Hier werden mehrere Variablen (showNotification, setCartSearch, cartSearch, searchText und categor) aus dem SearchContext abgerufen, die als Signals implementiert sind und im localStorage des Browsers gespeichert werden, da diese Daten zwischen verschiedenen Komponenten geteilt werden müssen.
...
Folgende Grafik gibt einen Überblick, wie die Komponenten zum Anzeigen der gesuchten Aufgaben (FindExBySearchText, FindExSubCat) auf die Variablen im SearchProvider zugreifen können:

\centfig{0.45}{suchen.png}

Folgende Grafik gibt einen Überblick, wie die Komponenten  zum Hinzufügen und Editieren von Aufgaben (ExForm, EditForm) auf die Variablen im SearchProvider zugreifen können:
\centfig{0.3}{hinzufügen.png}

Folgende Grafik gibt einen Überblick, wie die Komponenten zum Suche nach Aufgaben (SearchKorb) auf die Variablen im SearchProvider zugreifen können:
